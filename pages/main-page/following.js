...
export default function FollowingPage() {
  ...
  const fetchFollowingPosts = async (currentUser) => {
    try {
      setLoading(true);
      ...
      const followingUserIds = currentUser.following.join(',');
      const res = await fetch(`/api/posts/following?users=${followingUserIds}`, {
        credentials: 'include'
      });
      ...
      if (res.ok) {
        ...
        // Fallback: get all posts and filter by followed users
        const allPostsRes = await fetch('/api/posts?limit=50', {
          credentials: 'include'
        });
        
        if (allPostsRes.ok) {
          const allData = await allPostsRes.json();
          const followingPosts = allData.posts.filter(post => 
          ...
          <div className={styles.mainContent}>
            ...
            {posts.length === 0 ? (
              <div className={styles.noPosts}>
                ...
                <Link href="/main-page">
                  <button className={styles.createFirstPost}>
                    Explore Posts
                  </button>
                </Link>
              </div>
                        ...
                        <div className={styles.authorAvatar}>
                          {post.author?.avatar ? (
                            <img 
                              ...
                              alt={post.author.fullName || post.author.fullname}
